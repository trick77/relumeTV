...
func TestCreateEntertainmentConfig_payloadAndID(t *testing.T) {
	...
	// When
	id, err := testClient(srv).CreateEntertainmentConfig("relume-tv", []ConfigMember{
	...
	if gotBody["metadata"].(map[string]any)["name"] != "relume-tv" {
...
func TestGetEntertainmentConfig_channels(t *testing.T) {
	...
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"errors":[],"data":[{
			"id":"cfg-1","metadata":{"name":"relume-tv"},"status":"inactive",
			"channels":[
