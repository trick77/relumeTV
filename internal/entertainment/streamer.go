...
// configName is the metadata.name relume-tv uses for its own entertainment
// configuration on the Pro, so it can find and reuse it across restarts.
const configName = "relume-tv"

// sendInterval is the steady rate at which relume-tv re-sends the latest frame to the
...
// ProStreamer owns relume-tv's own entertainment stream to the Hue Bridge Pro. On a TV
// stream (OnStreamStart) it ensures+starts a relume-tv entertainment_configuration,
...
type ProStreamer struct {
	...
	// DTLS (the 50 Hz sendLoop). Lets the web UI show the live relume-tv→Pro send rate,
	// the upsampled counterpart to the TV→relume-tv input rate. Wired by main.
	OnSend func()
	...
	// loadCfgID / saveCfgID persist the resolved relume-tv config id across restarts
...
// SetConfigStore wires optional persistence of the resolved relume-tv config id (Phase
...
// establish ensures the relume-tv entertainment_configuration exists and is started,
...
func (s *ProStreamer) establish(ctx context.Context) error {
	...
	if err := pro.StartStream(configID); err != nil {
		// A reused config can be left active=true if relume-tv restarted mid-stream
...
// ensureConfig finds the relume-tv entertainment_configuration (or creates one
...
func (s *ProStreamer) ensureConfig(pro ProClient) (id string, remap map[uint16]uint8, reused bool, channels int, err error) {
	...
	// Honor the TV's group membership: if the TV told relume-tv which lights belong to
	...
	// and pick the persisted id if still present, else a config named `relume-tv`.
	...
	if id != "" {
		...
		default:
			...
			// would mint a duplicate `relume-tv` config); fail so run() retries via the
